...
class Workshops(db.Model):
    ...
    invitees = db.relationship("WorkshopInvitees", foreign_keys="WorkshopInvitees.workshop_id",lazy="joined")
    challenges = db.relationship("WorkshopChallenges", foreign_keys="WorkshopChallenges.workshop_id",lazy="joined")

    def __init__(self, *args, **kwargs):
        super(Workshops, self).__init__(**kwargs)

    def __repr__(self):
        return "<Workshops %r>" % self.name

class WorkshopInvitees(db.Model):
    __tablename__ = "workshop_invitees"
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(
        db.Integer, db.ForeignKey("users.id", ondelete="CASCADE")
    )
    workshop_id = db.Column(
        db.Integer, db.ForeignKey("workshops.id", ondelete="CASCADE")
    )
    user = user = db.relationship("Users", foreign_keys="WorkshopInvitees.user_id",lazy="joined")
    def __init__(self, *args, **kwargs):
        super(WorkshopInvitees, self).__init__(**kwargs)

class WorkshopChallenges(db.Model):
    __tablename__ = "workshop_challenges"
    id = db.Column(db.Integer, primary_key=True)
    challenge_id = db.Column(
        db.Integer, db.ForeignKey("challenges.id", ondelete="CASCADE")
    )
    ...
    challenge = db.relationship("Challenges", foreign_keys="WorkshopChallenges.challenge_id",lazy="joined")
    def __init__(self, *args, **kwargs):
        super(WorkshopChallenges, self).__init__(**kwargs)
