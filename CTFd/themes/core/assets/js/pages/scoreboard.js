...
const updateScores = () => {
  CTFd.api.get_scoreboard_list().then(response => {
    ...
    if(scoreids.length>0) {
      const ids = JSON.parse(scoreids.attr('class'))
      teams = teams.filter(function(team){return ids.includes(team.id)})
    }
...
const buildGraphData = () => {
  return CTFd.api.get_scoreboard_detail({ count: 500 }).then(response => {
    const places = response.data;

    let teams = Object.keys(places);
    if (teams.length === 0) {
      return false;
    }
    const option = {
    ...
    if(scoreids.length>0) {
      const ids = JSON.parse(scoreids.attr('class'))
      teams = teams.filter(function(team){ return ids.includes(places[team].id)})
    }
    for (let i = 0; i < teams.length; i++) {
      const team_score = [];
      const times = [];
